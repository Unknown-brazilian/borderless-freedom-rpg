...
## Final Sovereign Individual — campanha completa sem aceitar nenhum evento.
...
func _ready() -> void:
	...
	AudioManager.music("ending_sovereign")
	_btn_menu.pressed.connect(_on_menu)

	var nome := PlayerStats.player_name if PlayerStats.player_name != "" else "Dissidente"
	_lbl_name.text = nome + " — SOVEREIGN INDIVIDUAL."
	_lbl_sats.text = "%s sats — stack soberano (+50%% bônus aplicado)" % _fmt(SatEconomy.current_sats)
	...
	if _lbl_stats:
		_lbl_stats.text = (
			...
			+ "%d dias de viagem\n"
			+ "%d fiscais pagos\n"
			+ "%d armadilhas cripto detectadas e neutralizadas"
